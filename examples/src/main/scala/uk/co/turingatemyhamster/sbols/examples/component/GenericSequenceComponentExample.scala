 ...
 */
object GenericSequenceComponentExample {
  def main(args: Array[String]) {
    val gs1 = GenericSequence(
      identity = URI("http://turingatemyhamster.co.uk/example#gs1"),
      primarySequence = "lalala",
      legalCharacters = "la"
    )

    val gc1 = GenericSequenceComponent(
        identity = URI("http://turingatemyhamter.co.uk/example#gc1"),
        name = Some("Generic Component 1"),
        description = Some("My first Random Polymer Component"),
        displayId = Some("gc1"),
        componentType = URI("http://turingatemyhamster.co.uk/example#random_polymer"),
        sequence = Some(Reference(gs1.identity))
    )

    val gc2 = GenericSequenceComponent(
      identity = URI("http://turingatemyhamter.co.uk/example#gc2"),
      name = Some("Generic Component 2"),
      description = Some("My second Random Polymer Component"),
      displayId = Some("rc2"),
      componentType = URI("http://turingatemyhamster.co.uk/example#random_polymer"),
